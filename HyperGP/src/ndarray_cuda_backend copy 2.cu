...
void TEMPLATE_BIND_FUNCS_DIM1(py::module& m){
    ...
    //     int32_t* idxs_gpu;
    //     mem_pool_alloc_async(sizeof(int32_t) * idxs.size(), (void**)&idxs_gpu, out.device_id, out.stream_id);
...
using namespace gpu;
using namespace pygp_tensor;
void TEMPLATE_FUNCS_BROADCAST(py::module& m){
...
void TEMPLATE_FUNCS_EWISE(py::module& m){
    
    
    register_combinations([&m](auto type1){
        m.def("wait", [](Array<type1>& out){
            if (out.stream_id != -1){
    ...
    register_combinations([&m](auto type1, auto type2, auto type3){
        m.def("ewise_sub", [](Array<type1>& a, Array<type2>& b, Array<type3>& out, int32_t& offset_a, int32_t& offset_b){
            ewise_compute_2op<type1, type2, type3>(a, b, out, offset_a, offset_b, 1);
        });
    }, SupportedTypes{}, SupportedTypes{}, SupportedTypes{});
    ...
    register_combinations([&m](auto type1, auto type2){
