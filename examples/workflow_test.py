...
pop.initPop(pop_size=pop_size, prog_paras=ProgBuildStates(pset=pset, depth_rg=[2, 6], len_limit=256), parallel=False)
...
def selection(p1, p2, f1, f2):
    ...
    sample_list = [list(random.sample(legal_list, 3)) for i in p1]
    tour_list = [x[int(HyperGP.tensor.argmin(f_list[x]))] for x in sample_list]
